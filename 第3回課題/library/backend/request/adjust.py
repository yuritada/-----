from request import request_weather_data

def adjust_weather_data_for_weather_day_table(response):
    return_data = []
    for i in range(len(response[0]["timeSeries"][0]["timeDefines"])):
        for j in range(len(response[0]["timeSeries"][0]["areas"])):
            ...
            data["report_datetime"] = response[0]["reportDatetime"][i]
...
def adjust_weather_data_for_weather_day_pops_table(response):
    return_data = []
...
def adjust_weather_data_for_weather_day_temps_table(response):
    return_data = []
...
if __name__ == "__main__":
    office_code , response = request_weather_data("015000")
    print(adjust_weather_data_for_weather_day_table(response))
    print(adjust_weather_data_for_weather_day_pops_table(response))
    print(adjust_weather_data_for_weather_day_temps_table(response))